...
// AJAX 객체인 XHR을 생성하는 함수 - 크로스 브라우징 처리가 필요하기 때문에
...
// 응답 완료 시 데이터 처리함수로 사용
function callback(){
// 	console.log("callback called");// 총 4번 나와야한다, 상태가 4번 바뀌기 때문에
	
	if(xmlHttp.readyState == 4){ // readyState 4, DONE, 응답완료
		if(xmlHttp.status == 200){ // OK.(정상응답코드) HTTP status code 요청 처리코드를 의미
			console.log("정상 응답")
