...
public class FileServiceImpl implements FileService {
	...
	public void fileupload(HttpServletRequest req, HttpServletResponse resp) {
		...
		// 3. 업로드된 파일아이템의 용량이 적당히 작으면 메모리에서 처리하도록 설정
		...
		// 5. 업로드 허용 용량 기준을 넘지 않을 경우에만 파일업로드 처리되도록 설정
		//	-> 기준을 넘으면 업로드 차단됨
...
//		upload.setSizeMax(sizeMax);
		...
		// 7. 파싱될 요청정보 데이터 처리하기
		//	item 리스트에 요청파라미터(파일 포함)이 파싱되어 들어있음
...
//				}
			...
			if( !item.isFormField() ) {
				...
				// 2) 테이블의 컬럼으로 파일의 내용을 자장
				//	BLOB 타입을 사용한다 
				//	그러나 약간 성능 저하가 있음(DB가 바쁘기 때문..)
