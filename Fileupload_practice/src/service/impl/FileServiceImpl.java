...
public class FileServiceImpl implements FileService {
	...
	public void fileupload(HttpServletRequest req, HttpServletResponse resp) {
		...
		// 파일업로드 형태가 데이터가 맞는지 검사
		//	요청메시지 content-type 이 multipart/form-data가 맞는지 확인
		boolean isMultipart = false;
		isMultipart = ServletFileUpload.isMultipartContent(req);
		
		// 1-1 multipart/form-data 인코딩으로 전송되지 않았을 경우 처리 중단
		if( !isMultipart ) {
		...
		// ItemFactory : 엽로드된 데이터(FileItem) 을 처리하는 방식을 설정하는 클래스
		// FileItem : 클라이언트로부터 전송된 데이터를 객체화시킨 것
		
		// DiskFileitemFactory class
		// -> 디스크(HDD)기반의 파일아이템 처리API
		// -> 업로드된 파일을 디스크(HDD)에 임시저장하고 나중에 처리한다
		DiskFileItemFactory factory = new DiskFileItemFactory();
		
		// 3. 업로드된 파일아이템의 용량이 적당이 작으면 메모리에서 처리하도록 설정한다.
...
//		upload.setSizeMax(sizeMax);
		...
		// 7. 파싱될 요청정보 데이터 처리하기
		//	item 리스트에 요청파라미터(파일포함)이 파싱되어 들어있음
...
//				}
			...
			if( !item.isFormField()) {
				...
				// BLOB 타입을 사용한다
				// 그러나 약간 성능저하가 있음(DB가 바쁘기 때문에)
				
				// 우리는 1번을 사용
				
				//----------------------------------------------------
				
				// 서버에 저장되는 파일명을 "년원일시분초밀리초.확장자"로 변경하기
